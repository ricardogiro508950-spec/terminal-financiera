...
st.set_page_config(
    page_title="Terminal Financiera Institucional v5.11", page_icon="📊", layout="wide"
)

# AUTO-RECARGA AUTOMÁTICA CADA 15 SEGUNDOS (SIN BLOQUEAR YAHOO FINANCE)
st.markdown(
    """
...
st.title("📊 Terminal Financiera Institucional v5.11")
st.caption(
    "Panel Cuantitativo en Vivo | Auto-Actualización Automática y Nube"
)
st.markdown("---")
...
try:
    ...
    is_market_open = is_weekday and (
        9 <= market_hour < 16 or (market_hour == 9 and market_minute >= 30)
    )
    session_status = (
        "🟢 MERCADO ABIERTO (Wall Street)"
        if is_market_open
        else "🔴 MERCADO CERRADO (Fuera de Sesión)"
    )
except Exception:
...
with col_reloj1:
    st.markdown(
        f"🕒 **Hora Oficial NY (Wall Street):** `{ny_time_str}` — *{ny_date_str}*"
    )
with col_reloj2:
    ...
    for name, ticker in tickers.items():
        try:
            ...
            if not df.empty:
                ...
                prev_price = (
                    df["Close"].iloc[-2] if len(df) >= 2 else current_price
                )
...
for asset_name, info in market_data.items():
    ...
    if chg >= 0.8:
        active_alerts.append(
            f"🚀 **CRECIMIENTO DESTACADO:** En la sesión de Wall Street, **{asset_name}** registra un fuerte impulso alcista del `+{chg:.2f}%`."
        )
    elif chg <= -0.8:
        active_alerts.append(
            f"🔻 **CORRECCIÓN DETECTADA:** En la sesión de Wall Street, **{asset_name}** presenta una presión bajista del `{chg:.2f}%`."
        )
...
if active_alerts:
    ...
    st.info(
        "ℹ️ **Monitoreo Activo:** Los mercados principales operan con variaciones estables en este intervalo. Sin picos de volatilidad extrema."
    )
...
with ac_col1:
    capital = st.number_input(
        "Capital Total (USD)", min_value=10.0, value=1000.0, step=100.0
    )
with ac_col2:
    riesgo_pct = st.slider("Riesgo por Operación (%)", 0.5, 5.0, 1.0, 0.5)
with ac_col3:
    stop_loss_pct = st.number_input(
        "Stop-Loss Distancia (%)", min_value=0.1, value=5.0, step=0.5
    )

riesgo_usd = capital * (riesgo_pct / 100)
tamano_posicion = (
    riesgo_usd / (stop_loss_pct / 100) if stop_loss_pct > 0 else 0
)
...
st.caption(
    "Regla institucional: Nunca comprometas liquidez sin medir el impacto de una pérdida en el patrimonio total."
)
st.markdown("---")
...
with col3:
    render_mobile_card(
        col3, "DXY", dxy_info["price"], dxy_info["change"], is_currency=False
    )
with col4:
    render_mobile_card(
        col4,
...
dxy_status = (
    "BUENO. Inyecta liquidez a los activos de riesgo."
    if dxy_chg < 0
    else "PRECAUCIÓN. Fortaleza del dólar ejerce presión bajista."
)
bond_status = (
    "MALO para el riesgo."
    if bond_chg > 0
    else "FAVORABLE para los activos de riesgo."
)
rsi_status = (
    "SOBRECOMPRADO (>70). Alerta de posible corrección."
    if current_rsi > 70
    else (
        "SOBREVENDIDO (<30). Posible zona de rebote."
        if current_rsi < 30
        else "SANO. Subiendo o bajando de forma orgánica."
    )
)
ema_status = (
    "Precio operando por encima de la EMA 50. Tendencia alcista activa."
    if current_close > current_ema50
    else "Precio atrapado debajo de la EMA 50. PRECAUCIÓN. Resistencia activa."
)

st.markdown(
    f"* **El Viento a Favor (Macro):** El Dólar varía ({dxy_chg:.2f}%). {dxy_status}"
)
st.markdown(
    f"* **Tasas de Interés:** El Bono a 10 Años varía ({bond_chg:.2f}%). {bond_status}"
)
...
if current_close > current_ema50 and current_rsi < 70:
    st.success(
        "🟢 **ESTADO VERDE:** Alta confluencia alcista. Condiciones técnicas favorables para operar."
    )
elif current_close < current_ema50 and current_rsi > 30:
    st.warning(
        "🟡 **ESTADO AMARILLO:** Señales divididas. Mercado dudoso, mantén liquidez."
    )
else:
    st.error(
        "🔴 **ESTADO ROJO:** Alta volatilidad o zona de extremidad técnica. Riesgo elevado."
    )
...
st.caption(
    "Registra tus compras aquí. El sistema cruzará tus datos con el mercado en vivo para calcular tus ganancias o pérdidas reales."
)
...
def get_sheet_data():
    try:
        ...
        sheet = client.open_by_url(
            "https://docs.google.com/spreadsheets/d/1k-H50JiL6U41E6ne8qcmHeSvaoC8HCTe9DqWIQlP-Xo/edit"
        ).sheet1
...
if worksheet is None:
    st.error(
        "⚠️ No se pudo conectar a Google Sheets. Verifica los Secretos en Streamlit."
    )
...
with st.form("registro_operacion", clear_on_submit=True):
    ...
    with col_c:
        nueva_cantidad = st.number_input(
            "Cantidad", min_value=0.00001, format="%.5f"
        )
    with col_d:
        raw_precio = (
            btc_info["price"]
            if nuevo_activo == "Bitcoin"
            else gold_info["price"]
        )
        precio_seguro = (
            float(raw_precio)
            if raw_precio > 0
            else (60000.0 if nuevo_activo == "Bitcoin" else 2000.0)
        )
        nuevo_precio = st.number_input(
            "Precio Compra ($)", value=precio_seguro, min_value=0.1, format="%.2f"
        )
...
if not df_trades.empty and "Activo" in df_trades.columns:
    ...
    df_trades["Precio_Entrada"] = pd.to_numeric(
        df_trades["Precio_Entrada"], errors="coerce"
    )
    df_trades["Inversion_Inicial_USD"] = pd.to_numeric(
        df_trades["Inversion_Inicial_USD"], errors="coerce"
    )

    precios_actuales = {"Bitcoin": btc_info["price"], "Oro": gold_info["price"]}
    df_trades["Precio_Actual_Mercado"] = df_trades["Activo"].map(
        precios_actuales
    )
    df_trades["Valor_Actual_USD"] = (
        df_trades["Cantidad"] * df_trades["Precio_Actual_Mercado"]
    )
    df_trades["Ganancia/Perdida_USD"] = (
        df_trades["Valor_Actual_USD"] - df_trades["Inversion_Inicial_USD"]
    )
    ...
    rendimiento_total = (
        (ganancia_neta / inversion_total) * 100 if inversion_total > 0 else 0
    )
    ...
    st.info(
        "No tienes operaciones registradas. Ingresa una compra en el formulario de arriba."
    )
